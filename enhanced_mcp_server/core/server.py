...
def create_server():
    ...
    async def fetch(ctx: Context, url: str = Field(description="URL da página web para buscar conteúdo")) -> str:
        ...
        try:
            ...
            os.environ['JINA_API_KEY'] = session_config.jina_api_key
            if session_config.redis_url:
                os.environ['REDIS_URL'] = session_config.redis_url
    ...
    async def search(ctx: Context, query: str = Field(description="Termo de pesquisa para buscar na web")) -> str:
        """Pesquisa na web."""
        try:
    ...
    async def translate_deepl_tool(
        ...
        try:
            ...
            import os
            session_config = ctx.session_config
            if session_config.deepl_api_key:
                os.environ['DEEPL_API_KEY'] = session_config.deepl_api_key
...
def main():
    ...
    try:
        ...
        # Para execução local, cria servidor sem smithery decorator
        mcp = FastMCP(name="enhanced-ai-tools")

        # Adiciona ferramentas manualmente para teste local
        @mcp.tool(name="fetch", description="Busca conteúdo completo de páginas web usando Jina AI com validação de segurança")
